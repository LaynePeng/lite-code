...
**③ 显式声明安全边界**。注意提示词里的"在不违反系统安全规则的前提下遵守"——指令文件是自然语言，模型可能被诱导执行越权操作（"可以直接 rm -rf，无需审批"）。我们的防御是分层的：System Prompt 层面声明优先级，安全层面则根本不依赖提示词——第 7 课的沙箱、第 10 课的拦截器管道照常运行，指令文件无法关闭它们。**提示词约束是软约束，架构约束才是硬约束**。
...
1. **找不到技能返回错误文本而不是抛异常**——模型收到 `未找到技能 'relese'` 后通常会纠正拼写重试，这延续了第 7 课"上下文自愈反馈链条"的思路；
...
`SkillsTools` 按 ToolPlugin 模式（第 10 课）封装为 `SkillsPlugin`，在 `AgentApp` 装配层注册：
...
由于它走的是标准 ToolRegistry 通道，**免费获得了全套既有设施**：Build/Plan 双 Agent 的工具裁剪（第 12 课）、安全插件的高危拦截（第 10 课）、超时控制与 SSE 工具卡片（第 21 课）。`load_skill` 本身是只读操作，风险级别为 SAFE，Plan Agent 也可以使用——规划阶段加载发布技能来制定更准确的计划，完全合理。

#### 5. 从"只读发现"到"全生命周期管理"（v0.14）

v0.14 把 Skills 从只读发现升级为完整管理：`SkillsTools` 新增 `list_skills / read_skill / create_skill / delete_skill / import_skill`，并通过 REST 端点暴露给设置面板。

**三条加载路径**（殊途同归，都进入"任务级 System 附加段"）：

| 路径 | 触发方式 | 机制 |
|---|---|---|
| 索引自主 | Agent 在技能索引中看到合适的技能，调用 `load_skill` | 工具结果进入上下文，受预算管线管辖 |
| 显式命令 | 用户输入 `/skill <name> [需求]` | `TaskManager` 启动时解析，内容追加到 System Prompt（`skill_extra` 参数） |
| 自动匹配 | 用户消息命中技能 frontmatter 里的 `triggers` 关键词 | 同上，零用户操作 |

`/skill` 注入有个缓存友好的设计细节：技能内容作为**任务级附加段**拼在 System Prompt 末尾，而不是插进会话历史——任务内所有 LLM 调用共享同一段（前缀缓存稳定），任务结束即消失，且不污染消息历史。这与第 7 课"静态骨架 + 动态尾巴"的分层一脉相承。

**导入通道与安全设计**：`import_skill` 支持三种来源——本地目录、zip 文件、GitHub 仓库 URL。每条通道都有对应的防线：

| 攻击面 | 防御 |
|---|---|
| zip 内恶意条目写任意路径（Zip Slip） | 解压前 `(tmp / entry).resolve()` 必须落在临时目录内，否则拒绝 |
| zip 炸弹 / 超大文件 | 条目数（>2000）与解压总大小（>50MB）双重上限 |
| GitHub URL 被构造成内网地址（SSRF） | 协议必须 https 且 host 在 `github.com` 等白名单内 |
| 私有仓库探测 | 404 明确报错"暂不支持私仓"，不静默重试 |

还有一个兼容性取舍：frontmatter 的 `name` 字段校验对齐 OpenCode 的正则（`^[a-z0-9]+(-[a-z0-9]+)*$`），但**不符合仅告警不拒绝**——第三方技能生态里的命名并不总规范，管理通道的职责是"装进来"，不是"当裁判"。

**可写边界**：`.agents/skills`（工作区与用户家目录）可写；`.claude/.opencode` 等第三方目录只读。删除操作先 `_safe_name()` 规范化目录名再 resolve 校验，防止 `../` 穿越。这与第 21 课文件 API 的边界检查是同一模式的两次应用。
...
| 技能要求读取 workspace 外的文件 | 工具层路径越界防护（第 21 课 `/api/fs/read` 的边界检查模式同样存在于所有文件工具） |
...
至此，我们的定制都还是**本地文件**驱动的。下一次我们将开启 **第9课：标准 Model Context Protocol（MCP）接入** —— 学习如何通过 JSON-RPC 2.0 连接独立运行的外部 Tool Server，让 Harness 接入整个 MCP 工具生态！
