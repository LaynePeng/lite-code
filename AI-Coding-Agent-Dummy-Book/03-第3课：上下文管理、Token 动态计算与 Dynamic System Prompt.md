在实际的 Code Agent 开发中，随着对话轮数变多以及读取的文件变大，很快就会遇到 **Context Window Overflow（上下文窗口溢出）** 报错。更糟糕的是，当上下文过长时，LLM 会出现"注意力幻觉"与**上下文腐败（Context Rot）**，开始忽视最开始的指令或报错信息。
...
**为什么保留最近 K 轮？** 工具调用是链式推理（读了文件才能改、改了才能验证），删掉最近的决策链会破坏当前任务的连续性。`keep_recent_full_turns=2` 是实践中的经验值：既保住当前任务的连续推理，又能把更早的历史压缩成"对话主干"。
