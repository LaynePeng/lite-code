...
> 此处的动态组装是**概念演示**——它揭示了"环境感知"的价值，也埋下了与缓存前缀的冲突（见下文「设计决策」）；实战落地时我们把它收敛为**静态骨架 + 工具按需获取**（第 17 课 §3）。
...
本教程的实战落地（第 17 课）最终选择：**静态骨架 + 工具按需获取**。理由：
...
这个取舍值得记录：**「保新鲜」还是「保缓存」**——答案不是二选一，而是**区分静态与动态**：静态部分进 System 吃缓存，动态部分交给工具实时查询。这与第 4 课「稳定前缀」原则、第 17 课 AgentLoop 的静态 System Prompt 实现一脉相承。
