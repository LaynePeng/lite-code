...
本课编写 Code Agent 的代码感知系统 —— **代码检索与结构感知系统**：
...
在这节课中，我们为 Agent 实现了高效读取大型代码库的代码感知能力：
- 实现了 **Ripgrep 底层加速与正则搜索**；
- 实现了 **自动 Gitignore 过滤的文件树遍历器**；
- 增加了 **带行号标记与路径穿越防护** 的文件读取机制。

下一课进入 **第7课：基于 Tree-sitter 的抽象语法树（AST）分析与代码依赖图构建** —— 学习如何超越简单的文本搜索，从结构语义层面准确提取类、函数签名与调用链。