...
`/skill` 注入有个缓存友好的设计细节：技能内容作为**任务级附加段**拼在 System Prompt 末尾，而不是插进会话历史——任务内所有 LLM 调用共享同一段（前缀缓存稳定），任务结束即消失，且不污染消息历史。这与第 9 课"静态骨架 + 动态尾巴"的分层一脉相承。
...
**可写边界**：`.agents/skills`（工作区与用户家目录）可写；`.claude/.opencode` 等第三方目录只读。删除操作先 `_safe_name()` 规范化目录名再 resolve 校验，防止 `../` 穿越。这与第 20 课文件 API 的边界检查是同一模式的两次应用。
