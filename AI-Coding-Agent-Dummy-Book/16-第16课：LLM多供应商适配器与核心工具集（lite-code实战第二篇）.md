...
我们手写解析器，按 `index` 将 tool_calls 碎片增量拼接：
...
async def _parse_sse(self, response, events):
    ...
    async for chunk in response.aiter_bytes():
        ...
        for line in lines:
            ...
            if line == "data: [DONE]":
                return full_content, self._finalize(tool_calls_map)
    ...
    return full_content, [c for c in tool_calls_map.values() if c.name]
```
...
class AnthropicAdapter(BaseLLMAdapter):
    async def _parse_sse(self, response, events):
        ...
        async for chunk in response.aiter_bytes():
            # 解析 event: content_block_start / content_block_delta / content_block_stop
